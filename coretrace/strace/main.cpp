...
bool read_system(FILE* fp, st_context_t cxt, const char* base_dir = nullptr)
{
    if (!fp) return false;
...
int main(int argc, char *argv[])
{
  ...
  if (argc < 7)
  {
    printf("strace: too few arguments.  usage:\n\t"
      "strace.exe <system.stinput> <number of rays> <max rays> <seed> <sunshape? 0/1> <errors? 0/1> <pointfocus? 0/1> <refactor? 0/1> <embree? 0/1>\n");
    return -1;
  }
  ...
  int use_refactor = atoi(argv[8]);
  int use_embree = atoi(argv[9]);
  ...
  printf("input file: %s\n", file);
  if ( !read_system( fp, cxt, base_dir.c_str()) )
  {
  ...
  //int code = ::st_sim_run( cxt, (unsigned int)seed, trace_progress, 0 );

  int code = ::st_sim_run_with_refactor(cxt, (unsigned int)seed, trace_progress, 0,
      use_refactor, use_embree, nullptr);
