...
double Cylinder::z(double x, double) const
{
    ...
    //       Clyinder breaks the model since it is a mulit-valued fuction: each x values produces two z values
    //       Returning only the positive root
...
{
    // TODO: Double-check: This provides only the bottom-half of the sphere, aligning with the image in the documentation
    // TODO: Verify how vertex_curv relates to radius, i.e., vertex_curve = 1/r?
    return vertex_curv*(x*x + y*y) / (1 + sqrt(1 - vertex_curv*vertex_curv * (x*x + y*y)));
