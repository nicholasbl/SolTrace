...
TEST(SingleHeliostatSimulationNativeOptixComparison, SingleFacetFlat_North)
{
    ...
    EXPECT_NEAR(sim_native.helio_hit_count, sim_optix.helio_hit_count, 1000);
    EXPECT_NEAR(sim_native.reflect_count, sim_optix.reflect_count, 1000);
    EXPECT_NEAR(sim_native.helio_absorb_count, sim_optix.helio_absorb_count, 1000);
    EXPECT_NEAR(sim_native.rec_absorb_count, sim_optix.rec_absorb_count, 1000);
    EXPECT_NEAR(sim_native.rec_hit_count, sim_optix.rec_hit_count, 1000);
    EXPECT_NEAR(sim_native.rec_direct_hit_count, sim_optix.rec_direct_hit_count, 1000);
    EXPECT_NEAR(sim_native.rec_via_helio_hit_count, sim_optix.rec_via_helio_hit_count, 1000);
    ...
    EXPECT_NEAR((double)sim_native.helio_hit_count / n_records, (double)sim_optix.helio_hit_count / n_records, 0.01);
    EXPECT_NEAR((double)sim_native.reflect_count / n_records, (double)sim_optix.reflect_count / n_records, 0.01);
    EXPECT_NEAR((double)sim_native.helio_absorb_count / n_records, (double)sim_optix.helio_absorb_count / n_records, 0.01);
    EXPECT_NEAR((double)sim_native.rec_absorb_count / n_records, (double)sim_optix.rec_absorb_count / n_records, 0.01);
