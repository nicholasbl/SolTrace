...
TEST(Heliostat, SingleFacetFlat_North)
{
    constexpr uint_fast64_t NRAYS = 1e5; // 20e6;
    constexpr uint_fast64_t N_ABSORBED_THRESH = NRAYS / 10;
    ...
    Vector3d helio_origin(0.0, 500.0, 5.65);
    Vector3d rec_origin(0.0, 0.0, 169.0);

    auto hs = SolTrace::Data::make_element<Heliostat>();

    hs->set_optics(mirror, mirror_back);
    hs->set_reference_frame_geometry(helio_origin, khat, 0.0);
    hs->set_aperture_size(11.415, 10.42);   // Width, Height
    hs->set_number_panels(1, 1);
    hs->set_gaps(0, 0);
    hs->set_focal_length(0.0);
    hs->set_canting(Heliostat::NONE, 0.0, 0.0);
    hs->set_target_position(rec_origin);
    hs->create_geometry();
    hs->set_name("Heliostat");
    hs->enable();
    ...
    auto absorb = SolTrace::Data::make_element<SingleElement>();
    absorb->get_front_optical_properties()->set_ideal_absorption();
    absorb->get_back_optical_properties()->set_ideal_reflection();
    absorb->set_aperture(SolTrace::Data::make_aperture<SolTrace::Data::Rectangle>(12.0, 18.0));
    absorb->set_surface(SolTrace::Data::make_surface<SolTrace::Data::Flat>());
    Vector3d v1 = { 0.0, 1.0, 0.0 };
    Vector3d aim_point;
    vector_add(1.0, rec_origin, 1.0, v1, aim_point);
    absorb->set_reference_frame_geometry(rec_origin, aim_point, 0.0);
    absorb->set_name("Absorber");
    absorb->enable();
    auto rec_id = st2->add_element(absorb);
    EXPECT_TRUE(SolTrace::Data::Element::is_success(rec_id));
    ...
    hs->update_geometry(solar_azimuth, solar_elevation);
    ...
    EXPECT_EQ(sts, RunnerStatus::SUCCESS);
    result.write_csv_file("singlefacetflat_north_raydata_test.csv");
    ...
    double A_sun_box = sun_width * sun_height;
    double power_per_ray = A_sun_box / sys->SunRayCount * dni;
    ...
    // Calculate total power absorbed at the receiver and total rays hitting Heliostat
    uint_fast64_t count = 0;
    uint_fast64_t helio_hit_count = 0;
    uint_fast64_t reflect_count = 0;
    size_t n = ray_data->Count();
    ...
    double total_power = count * power_per_ray;
    double rec_hit_width = maxRecX - minRecX;
    double rec_hit_height = maxRecZ - minRecZ;

    EXPECT_TRUE(n >= NRAYS);
    EXPECT_EQ(NRAYS, helio_hit_count);
    EXPECT_NEAR((double) reflect_count / (double) helio_hit_count, 0.9, 1.e-3);
    ...
    //double power_tol = 1.e-4;       // NRAYS = 20,000,000
    double power_tol = 1.e-2;       // NRAYS = 100,000
    ...
    EXPECT_NEAR(total_power, expected_power, power_tol * expected_power);

    // TODO: Create a flux map and compare to expected distribution
    // TODO: Compare peak flux value to expected value
}