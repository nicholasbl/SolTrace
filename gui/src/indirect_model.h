...
template <class Key>
class HashContainerModel : public IndirectTableModel {
protected:
    QVector<Key> m_known_keys;

    // virtual bool _can_append_new(QVariant const&) = 0;
    // virtual bool _append_new(QVariant)            = 0;
    // virtual bool _can_delete_at(size_t, size_t)   = 0;
    // virtual void _delete_at(size_t, size_t)       = 0;
    int _record_count() const override { return m_known_keys.size(); }
