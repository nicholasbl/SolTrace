...
public:
     ...
     */
    Q_INVOKABLE QPointer<DocumentationContent> get(QString key);

private:
    void documentation_walker(const QString dir, const QString& key_prefix, const QString& section_number_prefix);
    QString locale_directory();

    QHash<QString, DocumentationContent*> m_docs_en; /// <- key → english docs
    QHash<QString, DocumentationContent*> m_docs_es; /// <- key → spanish docs
};
