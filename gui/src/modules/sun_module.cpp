...
bool CustomSunShapeModel::setData(const QModelIndex& index,
...
QHash<int, QByteArray> CustomSunShapeModel::roleNames() const {
    return { { AngleRole, "angle" }, { IntensityRole, "intensity" } };
}
...
void CustomSunShapeModel::copy_to_clipboard() {
    ...
    for (int i = 0; i < m_points.count(); i++) {
        text += QString::number(m_points[i].angle) + "\t" +
                QString::number(m_points[i].intensity) + "\n";
    }
