...
template <> struct std::hash<SD::OpticalProperties> {
...
template <> struct std::hash<db::MaterialComponent> {
...
template <> struct std::hash<db::GeometryComponent> {
...
template <> struct std::hash<SD::Aperture> {
  std::size_t operator()(SD::Aperture const &a) const {
    return hash_aperture(a);
  }
};

template <> struct std::hash<SD::Surface> {
  std::size_t operator()(SD::Surface const &a) const { return hash_surface(a); }
};
...
Database::Database(QObject *p)
    : QObject(p), m_registry(), identity(m_registry), transform(m_registry),
      global_transform(m_registry), invisible(m_registry), parent(m_registry),
      tag_root(m_registry), material_root(m_registry),
      material_parameters(m_registry), material_group_membership(m_registry),
      geometry_root(m_registry), geometry_parameters(m_registry),
      geometry_group_membership(m_registry), children(m_registry),
      tag_membership(m_registry), selected(m_registry), color(m_registry) {
...
static void import_optics(
    Database &reg, entt::entity entity, SD::Element const &item,
  ...
  if (auto group = material_groups.find(local_mat);
    ...
    registry.emplace<IdentityComponent>(
        group_entity, IdentityComponent{
...
void Database::import(SD::SimulationData &data) {
  ...
  for (auto iter = data.get_iterator(); !data.is_at_end(iter); ++iter) {
    ...
    import_optics(*this, ent, element, material_groups, geometry_groups,
                  group_counter);
  }

  {
    ...
    for (auto const &[e, stage] : view.each()) {
      ...
      auto new_pos = stage.stage_tf.position +
                     stage.stage_tf.rotation * stage.this_in_stage.position;
      auto new_rot = stage.stage_tf.rotation * stage.this_in_stage.rotation;
...
Database::operator entt::registry &() { return m_registry; }

Database::operator const entt::registry &() const { return m_registry; }

entt::registry &Database::as_registry() { return m_registry; }

const entt::registry &Database::as_registry() const { return m_registry; }
...
static void install_group(SD::element_ptr ptr, MaterialComponent const &param,
...
std::shared_ptr<SD::SimulationData> Database::export_to_simdata() {
...

  {
    ...
    for (auto const &[e, mat, geom] : view.each()) {
      ...
      auto const &mat_group = m_registry.get<MaterialComponent>(mat.group);
      auto const &geom_group = m_registry.get<GeometryComponent>(geom.group);
      ...
      if (auto ptr = m_registry.try_get<ChildrenComponent>(e); ptr) {
        if (!ptr->children.empty()) {
          ...
          auto composite = dynamic_cast<SD::CompositeElement *>(element.get());
          Q_ASSERT(composite);
...

  {
    ...
    for (auto const &[e, children] : view.each()) {
      ...
      auto composite =
          dynamic_cast<SD::CompositeElement *>(entity_element_map.at(e).get());
      Q_ASSERT(composite);
  ...
  for (auto const &iter : entity_element_map) {
    ...
    try {
      ...
      qCritical() << "Unable to export entity" << entt::to_integral(iter.first);
...
entt::entity Database::create() { return m_registry.create(); }

bool Database::valid(entt::entity e) const { return m_registry.valid(e); }
...
void Database::set_parent(entt::entity child, entt::entity parent) {
  ...
  if (!m_registry.all_of<ChildrenComponent>(parent)) {
    ...
    m_registry.patch<ChildrenComponent>(
        parent, [child](ChildrenComponent &a) { a.children.push_back(child); });
  }
...
void Database::assign_material(entt::entity child, entt::entity group) {
  if (!valid(child) or !valid(group))
    return;

  if (!m_registry.all_of<MaterialGroupComponent>(group))
    return;
  ...
  ::db::emplace_patch<MaterialGroupComponent>(
      m_registry, group,
      [child](MaterialGroupComponent &c) { c.members.push_back(child); });
}
...
void Database::assign_geometry(entt::entity child, entt::entity group) {
  ...
  if (!m_registry.all_of<GeometryGroupComponent>(group))
    return;
  ...
  ::db::emplace_patch<GeometryGroupComponent>(
      m_registry, group,
      [child](GeometryGroupComponent &c) { c.members.push_back(child); });
}
...
entt::entity Database::create_tag(QString name) {
  ...
  m_registry.emplace<IdentityComponent>(ret, IdentityComponent{.name = name});
...
void Database::assign_tag(entt::entity item, entt::entity tag) {
  if (!m_registry.all_of<TagComponent>(tag)) {
    return;
  }

  if (is_tagged(item, tag))
    return;
  ...
  ::db::emplace_patch<TagMembershipComponent>(
      m_registry, item,
      [tag](TagMembershipComponent &tc) { tc.tags.push_back(tag); });
}
...
void Database::unassign_tag(entt::entity item, entt::entity tag) {
  ...
  if (!m_registry.all_of<TagMembershipComponent>(item)) {
    return;
  }
...
void Database::delete_geometry_group(entt::entity to_delete,
  ...
  // if not a group, bail
  if (!m_registry.all_of<GeometryGroupComponent>(to_delete)) {
    return;
  }
