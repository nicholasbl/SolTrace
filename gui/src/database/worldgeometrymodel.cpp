...
void InstancedElements::on_geometry_group_change(entt::entity group) {
  if (group != entt::null && group != m_target_group)
    return;
  ...
  for (auto member : std::as_const(ptr->members)) {
    ...
    auto entry = calculateTableEntryFromQuaternion(
        convert(global->position), QVector3D(1, 1, 1),
        convert(global->rotation), color);
...
void InstancedElements::on_geometry_group_membership_change(
  ...
  if (m_target_group == entt::null)
    return;

  if (ptr->group != m_target_group)
    return;
...
entt::entity InstancedElements::entity_at(int index) {
  if (index < 0 || index >= m_member_cache.size()) {
    return entt::null;
  }
...
InstancedElements::InstancedElements(Database *db, entt::entity group,
                                     QQuick3DObject *parent)
    : QQuick3DInstancing(parent), m_database(db), m_target_group(group) {
  if (!db)
    return;

  connect(db->geometry_group_membership.self(), &ComponentAPIBase::changed,
          this, &InstancedElements::on_geometry_group_membership_change);

  connect(db->geometry_group_membership.self(), &ComponentAPIBase::removed,
          this, &InstancedElements::on_geometry_group_membership_change);

  connect(db->global_transform.self(), &ComponentAPIBase::changed, this,
          &InstancedElements::on_instance_changed);

  connect(db->geometry_root.self(), &ComponentAPIBase::changed, this,
          &InstancedElements::on_geometry_group_change);

  connect(db->geometry_root.self(), &ComponentAPIBase::removed, this,
          &InstancedElements::on_geometry_group_change);

  // Selection connections
  connect(db->selected.self(), &ComponentAPIBase::changed, this,
          &InstancedElements::on_geometry_group_change);

  connect(db->selected.self(), &ComponentAPIBase::removed, this,
          &InstancedElements::on_geometry_group_change);
...
QByteArray InstancedElements::getInstanceBuffer(int *instanceCount) {
  ...
  if (instanceCount) {
    *instanceCount = m_member_cache.size();
  }
...
void WorldGeometryModel::group_removed(entt::entity e) { recompute(); }
...
void WorldGeometryModel::reset(Database *database) {
  ...
  connect(database->geometry_root.self(), &ComponentAPIBase::changed, this,
          &WorldGeometryModel::group_changed);

  connect(database->geometry_root.self(), &ComponentAPIBase::removed, this,
          &WorldGeometryModel::group_removed);

  connect(database->identity.self(), &ComponentAPIBase::changed, this,
          &WorldGeometryModel::group_changed);
}
